...
import { SeriesByActorPageComponentState, SeriesByActorPageInfo } from '../models/series-by-actor-page.models';

@Component({
  selector: 'app-serie-by-actor',
  template: `
      ...
      <ng-container *ngIf="(info$ | async) as info">
        <ng-container *ngIf="status !== EntityStatus.Error; else serverError">
          ...
          <ng-container *ngIf="$any(info)?.series?.length > 0; else noData">
            <ng-container *ngFor="let serie of $any(info)?.series; trackBy: trackById">
  ...
  `,
  styleUrls: ['./serie-by-actor.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SerieByActorComponent {
  ...
  componentState!: SeriesByActorPageComponentState;
  triggerLoad = new EventEmitter<SeriesByActorPageComponentState>()
  cacheInfo!: SeriesByActorPageInfo;
  ...
  info$ = this.triggerLoad.pipe(
    switchMap(({idActor, slice, search, reload }) => {
      ...
      const { detail, series } = this.cacheInfo || {};
      const services$ = {
        responseDetail: !reload ? of(detail) : this.actorService.getDetail(idActor).pipe(catchError(() => of({}))),
        responseSeries: !reload ? of(series) : this.actorService.getSeries(idActor).pipe(catchError(() => of([])))
      };
      ...
      return forkJoin(
        ...
        map((response) => {
          const { responseDetail: detail, responseSeries: series } = response || {};
          ...
          const filterSeries = search
                              ? series?.filter(({name}) => name?.toLocaleLowerCase()?.includes(search?.toLocaleLowerCase()))
                              : series;
          return {
            detail,
            series: filterSeries?.slice(0, slice),
            total: filterSeries?.length
          }
        }),
        catchError(() => {
          this.status = EntityStatus.Error;
          this.notificationService.notificationFailure('ERRORS.ERROR_LOAD_TV');
          return of({detail:{}, series:[], total:0})
        })
        // ,tap(d => console.log(d))
      )
    })
  ...
  changeComponentState(search?: string, reload: boolean = false): SeriesByActorPageComponentState{
    return {
      ...this.componentState,
      slice: this.slice,
      search,
